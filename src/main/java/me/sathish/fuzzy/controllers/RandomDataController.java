...
public class RandomDataController {
    ...
    public List<AccountHolder> getAllAccounts() {
        ...
        List<Document> accountDocHolderList =
                accountHolderList.stream()
                        .map(
                                singleDoc -> {
        ...
        Map<String, List<Match<Document>>> result =
                matchService1.applyMatchByDocId(accountDocHolderList);
        result.entrySet()
                .forEach(
                        entry -> {
                            entry.getValue()
                                    .forEach(
                                            documentMatch -> {
                                                System.out.println(
                                                        "Data :"
                                                                + documentMatch.getData()
                                                                + "Matched with:"
                                                                + documentMatch.getMatchedWith());
                                            });
                        });
